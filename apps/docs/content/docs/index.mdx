...
Mycelia detects and creates relationships between nodes:

- **Contains**: Parent-child hierarchies
- **References**: Explicit connections between nodes
- **Mentions**: Content that mentions other entities
- **Tags**: Connections through shared tags

### 3. Knowledge Graph

Your content becomes a graph structure stored as JSON:
...
const people = graph.indexes.byType['person'].map(id => graph.nodes[id]);

// Navigate relationships via edges
const projectTasks = graph.edges
